  ...
  function checkTimeBiggerOrSmaler($time, $lifedays = false)
    ...
    function formatSize($size)
   {
    ...
    <div class="h-100 bg-cover bg-center py-5 d-flex align-items-center"
         style="background-image: url({{ uploaded_asset(get_setting('login_page_image')) }})">
    ...
    //                                        if (isset($datarray['attachmentsxx']) && !empty($datarray['attachmentsxx'])  && isset($datarray['attachments']) && !empty($datarray['attachments']) && checkTime($time, $lifetime)) {
    //                                        if (isset($datarray['attachmentsxx']) && !empty($datarray['attachmentsxx']) && checkTime($time, $lifetime)) {
    //                                            // value is not null
    //                                            $expired = false;
    ...
    //                                        if ((isset($datarray['attachmentsxx']) && !empty($datarray['attachmentsxx']) && checkTimeBetween($time, $lifetime)) ||(isset($datarray['attachmentsxx']) && !empty($datarray['attachmentsxx']) && checkTime($time, $lifetime))) {
    ...
    //                                            dd('valid  ======> $expired='.$expired.'  $linkkeyvalidate='.$linkkeyvalidate.'  $data_time_startx='.date('d-m-Y H:i:s', $data_time_startx).'  $data_time_endx='.date('d-m-Y H:i:s', $data_time_endx).'  $bigger_smaller='.$bigger_smaller.'  $value[idfilename]='.$value['idfilename'].'  $find_val='.$find_val.'  $datarray='.json_encode($datarray));
                                            } else {
                                                // value is null or empty
                                                $expired = true;
                                                $linkkeyvalidate = false;
                                                ...
                                                $lifedays = $lifetime ? (int)$lifetime : 1;
                                                $data_time_endx = (86400 * $lifedays) + $time;
    //                                            dd('invalid======> $expired='.$expired.'  $linkkeyvalidate='.$linkkeyvalidate.'  $data_time_startx='.date('d-m-Y H:i:s', $data_time_startx).'  $data_time_endx='.date('d-m-Y H:i:s', $data_time_endx).'  $bigger_smaller='.$bigger_smaller.'  $value[idfilename]='.$value['idfilename'].'  $find_val='.$find_val.'  $datarray='.json_encode($datarray));
                                            }
                                            break;
                                        } else {
                                            $datarray = [];
...
//                                        if ($mandil2) {
//        //                                    error_log("mandil2=> ".$mandil2."    session=> ".strtolower(Session('default_recaptcha_id_customer_login')));
//                                            if (strtolower($mandil2) != strtolower(Session('default_recaptcha_id_customer_login'))) {
//                                                $passa = false;
//                                                $passcap = false;
...
//                                        }
    ...
    //                                    $enable_prettylinks_config = false;
    //                                    $show_captcha_download_config = false;
        ...
        //                                $onetime_download = isset($datarray['onetime']) ? (int)$datarray['onetime'] : $one_time_download_config;
                                        ...
                                        $sh = md5($time . $hash);
//dd('$passcap='.$passcap .'  $mandil2='.strtolower($mandil2).'     default_recaptcha_id_customer_login='.strtolower(Session('default_recaptcha_id_customer_login')).'    $passa='.$passa);
//                                        if ($passa === true && $passcap === true) {
                                        if ($passa && $passcap) {
    //                                        $direct_links = false;
    //                                        $target = '';
...
//                                            if (checkTime($time, $lifetime) || checkTimeBetween($time, $lifetime)) {
                            ...
                            {{--                            <div class="col-lg-12">--}}
                            <div class="row shared-links" style="margin: 10px;">
                                @php
...
//                                                        $imgdata = 'data-ext="' . $extension . '"';
                                            ...
                                            $parameters = array();
        //                                        array_push($parameters, base64_encode($countfiles));
                                            array_push($parameters, $countfiles);
                                            $paramatts = join(',', $parameters);
        //                                        array_push($parameters, base64_encode($sh));
                                            array_push($parameters, $sh);
                                            $paramatts = join(',', $parameters);
        //                                        array_push($parameters, base64_encode($linkkey));
                                            array_push($parameters, $linkkey);
                                            $paramatts = join(',', $parameters);
        //                                        array_push($parameters, base64_encode($pezzo));
                                            ...
                                            <div class="col-md-6">
                                                ...
                                                <div class="btn-group">
                                                    ...
                                                    <span class="ms-auto small itemsize"
                                                          style="color:#ffffff;font-size:13px;text-align:left;height:18px;font-weight: bold">
                                                        <?php echo $filename; ?>
                                                    </span>
                                                                <span class="ms-auto small itemsize"
                                                                      style="color:#ffffff;font-size:13px;margin-left:10px; margin-right:10px; text-align:left;height:18px;">
                                                    ...
                                                    <div class="row">
                                                        ...
                                                        <div class="col-lg-6 mx-auto">
                                                            ...
                                                            <div class="text-center">
                                                                ...
                                                                {{--                                                    <h5 class="fs-14 fw-400 text-dark">{{ strtolower(Session('default_recaptcha_id_customer_login')) }}</h5>--}}
                                                            </div>

                                                            <div class="card">
                                                                {{--                                        <div class="card-header text-center">--}}
                                                                {{--                                            <h3 class="fs-18 mb-0">{{translate('Enter the required information')}}</h3>--}}
                                                                {{--                                        </div>--}}
                                                                                    ...
                                                                                    <div class="input-group">
                                                                                        ...
                                                                                        <div
                                                                                            ...
                                                                                            <a href="#"
                                                                                               ...
                                                                                               class="btn btn-info service-btn d-flex align-items-center justify-content-center">
                                                                                                {{--                                                                                                <i style="width:1.0rem; height:1.3rem;" class="las la-refresh la-2x"></i>--}}
                                                                                                {{--                                                                                                <i class="tio-refresh icon cursor-pointer p-2"></i>--}}
                                                        ...
                                                        <div class="col-lg-6 col-md-5 py-3 py-md-0">
                                                            <img
                                                                    src="{{ uploaded_asset(get_setting('login_page_image')) }}"
                                                                    alt=""
                                                                    class="img-fit h-100">
                                                        </div>
